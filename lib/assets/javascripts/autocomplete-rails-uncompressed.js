      ...
      jQuery(e).autocomplete({
        ...
        select: function( event, ui ) {
          ...
          if (e.delimiter != null) {
            ...
            if (jQuery(this).attr('data-id-element-diff')) {
              var parts = jQuery(this).attr('data-id-element-diff').split('/');
              jQuery('#' + jQuery(this).attr('id').replace(parts[0], parts[1])).val(ui.item.id);
            }
          ...
          jQuery(this).bind('keyup.clearId', function(){
            if(jQuery(this).val().trim() != remember_string.trim()){
              ...
              if (jQuery(this).attr('data-id-element-diff')) {
                var parts = jQuery(this).attr('data-id-element-diff').split('/')
                jQuery('#' + jQuery(this).attr('id').replace(parts[0], parts[1])).val("");
              }
